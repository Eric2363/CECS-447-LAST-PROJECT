...
void TCS34727_Init(void){
	...
	UART_OutString((uint8_t*)printBuf);
	
	if(ret != TCS34727_ID){
		UART_OutString((uint8_t*)"TCS34727 has not been Detected\r\n");
		return;
	}
	UART_OutString((uint8_t*)"TCS34727 has been Detected\r\n");
	...
	if(ret != 0)
		UART_OutString((uint8_t*)"Error on Transmit\r\n");
	else
		UART_OutString((uint8_t*)"TCS34727 Integration Time Set\r\n");
	...
	if(ret != 0)
		...
		UART_OutString((uint8_t*)"TCS34727 Gain Set\r\n");
	...
	if(ret != 0)
		...
		UART_OutString((uint8_t*)"TCS34727 Power On\r\n");
	...
	if(ret != 0)
		...
		UART_OutString((uint8_t*)"TCS34727 RGBC On\r\n");
	
	//Integration Time Delay when Activating. Varies with Integration Time Choosen by User
	DELAY_1MS(3);
	
	UART_OutString((uint8_t*)"TCS34727 Color Sensor Initialized\r\n");
